...
# pylint: disable=C0301,R0913,R0917,R0903
# ruff: noqa: E501
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        """`forward` method for the MLP module."""
        x = self.fc1(x)
        x = self.relu(x)
        x = self.dropout(x)
        x = self.fc2(x)
...
class MixerLayer(nn.Module):
    ...
    def __init__(
        self, num_features, num_time_steps, hidden_dim, dropout_rate
    ):
        """`MixerLayer` constructor."""
        super().__init__()
    ...
    def forward(self, x):
        """`forward` method for the `MixerLayer` module."""
        # Time Mixing
        x_residual = x
        x = self.norm_features(x)
        x = x.permute(
            0, 2, 1, 3
        )  # Transpose to (batch, time, features, channels)
        x = self.mlp_time(x)
        x = x.permute(
            0, 2, 1, 3
        )  # Transpose back to (batch, features, time, channels)
        x = x + x_residual
...
class TemporalProjection(nn.Module):
    ...
    def __init__(self, num_features, num_time_steps):
        """`TemporalProjection` constructor."""
        super().__init__()
        self.norm_time = nn.BatchNorm2d(num_time_steps)
        self.fc = nn.Linear(num_features, num_features)

    def forward(self, x):
        """`forward` method for the `TemporalProjection` module."""
        x = self.norm_time(x)
        x = x.permute(
            0, 2, 1, 3
        )  # Transpose to (batch, time, features, channels)
...
class TSMixer(nn.Module):
    ...
    def forward(self, x):
        """`forward` method for the `TSMixer` model."""
